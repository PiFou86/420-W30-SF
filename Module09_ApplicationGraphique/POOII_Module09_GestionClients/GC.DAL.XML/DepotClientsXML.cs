...
namespace GC.DAL.XML
{
    public class DepotClientsXML : IDepotClients
    {
        ...
        public List<Client> RechercherClients(string p_informationPartielle)
        {
            return this.ListerClientsDTO()
                .Where(client =>
                   client.Nom.Contains(p_informationPartielle, StringComparison.InvariantCultureIgnoreCase)
                || client.Prenom.Contains(p_informationPartielle, StringComparison.InvariantCultureIgnoreCase))
